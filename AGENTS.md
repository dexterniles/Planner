...
Today there is one allowed user; tomorrow there could be more. Resolve `userId` from the auth helper inside route handlers and route-called libs. Maintenance scripts (`scripts/seed.ts`, `scripts/import-letterboxd.ts`) read the target user from the `SEED_USER_ID` env var instead. To graduate to per-user data, drop the allowlist; no per-route refactor is required.
